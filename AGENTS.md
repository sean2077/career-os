...
- This public repository uses `development_topology = "standalone-framework"`. Framework work is implemented and validated here; real career records, personal identity, attachments, local fonts, active `.obsidian/` state, `runtime/`, `build/`, and `.career-os/` must not enter its tracked snapshot.
