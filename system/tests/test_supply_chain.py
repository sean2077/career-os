...
def test_sbom_matches_all_runtime_locks_and_external_assets() -> None:
    ...
    assert not any(str(item).startswith("skill:defuddle@") for item in references)
    assert not any(str(item).startswith("skill:opencli-usage@") for item in references)
